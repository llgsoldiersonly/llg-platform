...
export default async function ClientDeliverablesPage({
  ...
  const [{ data: client }, deliverablesRes, subscriptionsRes, staffRes] = await Promise.all([
    supa.from('clients').select('id, firm_name').eq('id', id).maybeSingle(),
    supa
      .from('deliverables')
      .select(`
        id, source, is_incentive, template_id, status, assigned_to, actual_count, period_start, period_end, notes,
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-8">
      ...
      <DeliverablesTabs deliverables={deliverables} clientId={id} staff={staff} />
    </div>
  )
}

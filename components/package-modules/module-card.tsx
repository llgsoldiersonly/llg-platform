    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {Object.keys(config).length > 0 && (
          <div className="rounded-md bg-neutral-secondary-soft p-3 text-xs text-body">
            <p className="mb-1 font-medium uppercase tracking-wide text-body">
              Plan limits
            </p>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-1">
              {Object.entries(config).map(([k, v]) => (
                <div key={k} className="contents">
                  <dt className="text-body">{k.replace(/_/g, ' ')}</dt>
                  <dd className="text-right text-heading">{String(v)}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        {deliverables.length === 0 ? (
          ...
          <ul className="space-y-2">
            {deliverables.map((d) => {
              ...
              return (
                <li key={d.id} className="rounded-md border border-border-light p-3">
                    ...
                    <div className="flex-1">
                      ...
                      {target && (
                        <div className="mt-2">
                          ...
                          <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-neutral-tertiary-soft">
                            <div
                              className="h-full bg-dark transition-all"
